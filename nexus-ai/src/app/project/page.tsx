...
import { JoinProjectCard } from "@/features/workspace/components/JoinProjectCard";
    ...
    <section className="space-y-6">
      <header className="relative overflow-hidden rounded-3xl border bg-slate-950 p-8 text-white shadow-xl">
        ...
        <div className="relative flex flex-col gap-4">
          ...
          <h1 className="mt-2 text-3xl font-black tracking-tight md:text-4xl">Quản trị project</h1>
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_340px]">
        {/* Left: Project List */}
        <section className="rounded-3xl border bg-white p-6 shadow-sm h-fit">
          ...
          {projects.length ? (
            <div className="mt-6 grid gap-4 sm:grid-cols-2">
              {projects.map((project) => (
                <Link
                  className="group rounded-2xl border border-slate-200 bg-gradient-to-br from-white to-slate-50 p-5 transition hover:-translate-y-1 hover:border-cyan-200 hover:shadow-xl hover:shadow-cyan-100"
                  href={`/project/${project.id}`}
                  key={project.id}
                >
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      <h3 className="truncate font-bold text-slate-950 group-hover:text-cyan-700">
                        {project.name}
                      </h3>
            ...
            <div className="mt-6 rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-8 text-center">
              ...
              <p className="mx-auto mt-2 max-w-md text-sm leading-6 text-slate-500">
                Tạo project mới hoặc tham gia bằng mã/invite link để bắt đầu.
              </p>
            </div>
          )}
        </section>
        ...
        <aside className="space-y-6">
          <div className="rounded-3xl border border-violet-100 bg-violet-50/40 p-6 shadow-sm">
            <div className="flex items-center gap-3">
              <span className="flex size-10 items-center justify-center rounded-2xl bg-violet-600 text-white shadow-md shadow-violet-100">
                <Plus size={18} />
              </span>
              <div>
                <h2 className="font-black text-slate-950 text-base">Tạo Project mới</h2>
                <p className="text-xs text-slate-500">Khởi chạy workspace riêng</p>
              </div>
            </div>
            <p className="mt-3 text-xs leading-relaxed text-slate-500">
              Trở thành PM, thiết lập deadline, nạp tài liệu và sử dụng AI Planner để chia việc tự động cho cả đội.
            </p>
            <Link
              className="mt-4 inline-flex w-full items-center justify-center gap-2 rounded-2xl bg-slate-950 py-3 text-sm font-bold text-white transition hover:bg-slate-800"
              href="/project/new"
            >
