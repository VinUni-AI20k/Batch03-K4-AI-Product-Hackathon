...
from tools._shared import ROOT, TIMEOUT
...
def get_arxiv_paper_text(arxiv_url: str = "", max_pages: int = 5, max_chars: int = 8000) -> dict[str, Any]:
    try:
        ...
        return {
            "tool": "get_arxiv_paper_text",
            "arxiv_url": arxiv_url,
            "status": "blocked" if isinstance(exc, ValueError) else "skipped",
            "skipped": True,
            "fallback_allowed": False,
            "error": type(exc).__name__,
            "message": f"The supplied arXiv reference could not be read and was skipped: {exc}",
            "items": [],
        }
