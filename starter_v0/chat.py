...
def tool_results_message(events: list[dict[str, Any]]) -> dict[str, str]:
    terminal_skip = any(
        ...
        and event["result"].get("fallback_allowed") is False
        for event in events
    )
    guardrail_instruction = ""
    if terminal_skip:
        guardrail_instruction = (
            " A supplied URL was blocked or skipped. This is terminal for that URL: do not search the web, "
            "Twitter/X, papers, or substitute another URL. Briefly report the skip and ask for a corrected URL if useful."
        )
    return {
        "role": "user",
        "content": (
...
def run_model_tool_loop(
    *,
    ...
    for round_index in range(1, max_tool_rounds + 1):
        ...
        completed_actions = [
            event for event in non_clarification_events
            if isinstance(event.get("result"), dict)
            and event["result"].get("action_completed") is True
        ]
        if completed_actions:
            # A side effect must be exactly-once. Return immediately instead of
            # giving the model another round in which it could repeat the send.
            messages = [
                str(event["result"].get("message") or "Action completed.")
                for event in completed_actions
            ]
            return {
        ...
        if terminal_skips:
            # Expose no tools for the terminal synthesis turn. This makes the
            # no-fallback rule enforceable even if a model would otherwise try
            # lookup, social search, or a replacement URL in a later round.
            final_response = provider.complete(working_messages, [], model=model, temperature=0.0)
            rounds.append({
                "round": round_index + 1,
            ...
            all_skipped = bool(non_clarification_events) and all(
                isinstance(event.get("result"), dict)
                and event["result"].get("fallback_allowed") is False
                for event in non_clarification_events
            )
            fallback_text = (
                "Không thể đọc URL đã cung cấp nên bài báo đã được bỏ qua. "
                "Tôi không tìm kiếm nguồn thay thế; hãy gửi một URL hợp lệ nếu bạn muốn thử lại."
            )
            return {
                "status": "guardrail_skipped" if all_skipped else "answered_with_skips",
                "assistant_text": final_response.text or fallback_text,
                "rounds": rounds,
                "tool_events": all_tool_events,
            }
