...
*(Không có worksheet dạng hình riêng — JTBD đã chốt bằng văn bản đầy đủ ở trên theo đúng cấu trúc `verb + object + bối cảnh` của guide §1.1 câu 2; guide không bắt buộc phải có bản vẽ nếu đã có văn bản.)*
...
Log: [`evidence/Hackathon_Idea_Survey_Submissions_2026-07-31.csv`](evidence/Hackathon_Idea_Survey_Submissions_2026-07-31.csv) (n=35 ≥ 20 ✓, log đầy đủ từng câu hỏi + từng câu trả lời có trong file ✓ — đã đối chiếu lại toàn bộ 35 dòng, mọi % trong bảng dưới khớp chính xác với dữ liệu thô)
...
4 thành viên mỗi người tự dùng thử 15 phút. Gợi ý phân sản phẩm theo vai trò đã có ở §8 để tận dụng góc nhìn:
...
| C | (4) Quiz UI + Rule-based Grading · (6) UI style/time (độc lập, chèn bất cứ lúc nào) · (10) Retest Grading + Result screen |
| Đoàn Ngọc Linh - 2A202601762 | (5) Learning Diagnosis (weak_sections + quyết định reteach/retest) · (7) Align weak sections ↔ transcript |
| E | (8) Roadmap + grounded content (Summary Card / ví dụ / Mini Practice Question, có source reference) |
...
|---|---|---|
| 22:12 [30/7] Khởi tạo spec từ problem-definition.md | Bản đầu tiên, chưa có evidence thật |
...
- [ ] Spec đủ §1-§9 theo template 
- [ ] Evidence đạt chuẩn A có log (khảo sát n=35, log CSV đầy đủ)
- [ ] Bảng impact ≥3 ứng viên + ứng viên loại có lý do
- [ ] ≥4 nguyên tắc HAX/PAIR có "áp vào đâu" cụ thể (đã có 6: G1, G2, G10, G11, G9, G8 + 2 PAIR)
- [ ] 4 lớp × ≥2 kịch bản = 9 kịch bản (đủ ≥8)
