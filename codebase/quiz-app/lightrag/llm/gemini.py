...
def _ensure_api_key(api_key: str | None) -> str:
    ...
    if not key:
        raise ValueError(
            ...
            "Set EMBEDDING_BINDING_API_KEY, GEMINI_API_KEY or LLM_BINDING_API_KEY in the environment."
        )
    return key
...
async def gemini_embed(
    ...
    async def _embed_single(text: str) -> np.ndarray:
        ...
        if config_obj is not None:
            req_kwargs["config"] = config_obj
        res = await client.aio.models.embed_content(**req_kwargs)
    ...
    results = await asyncio.gather(*[_embed_single(t) for t in texts])
