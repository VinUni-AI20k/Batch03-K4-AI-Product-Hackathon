...
class SlideSummaryAgent:
    """
    AI Agent phụ trách đọc, phân tích và tóm tắt file slide (PDF/PPTX).
    """
    ...
    def summarize_slide_file(
        ...
        print(f"🔄 [Agent] Đang đọc file slide: {file_name}...")

        slides = SlideParser.extract_slides(file_path)
        total_slides = len(slides)
        print(f"✅ [Agent] Đã trích xuất xong {total_slides} slide.")

        if total_slides == 0:
            return {"error": "Slide không có nội dung văn bản."}

        if total_slides <= map_reduce_threshold:
            print(f"🚀 [Agent] Sử dụng chiến lược Single-Pass Summarization...")
            full_text = SlideParser.get_full_text(slides)
            prompt = SLIDE_SINGLE_PASS_PROMPT.format(
                total_slides=total_slides,
                full_text=full_text
            )
            raw_summary = self.llm_client.generate(
                prompt=prompt,
                system_instruction=self.system_prompt
            )
            final_report = raw_summary
        else:
            print(f"🚀 [Agent] Slide dài ({total_slides} trang), sử dụng chiến lược Map-Reduce Summarization...")
            map_results = []
            for item in slides:
                ...
                print(f"   ↳ Đã tóm tắt Slide {s_num}/{total_slides}")
        ...
        if output_dir:
            ...
            print(f"💾 [Agent] Đã lưu báo cáo tóm tắt tại: {saved_file_path}")
