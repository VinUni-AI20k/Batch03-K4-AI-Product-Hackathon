...
# Load biến môi trường (.env) 1 LẦN DUY NHẤT lúc app khởi động - không được để việc này phụ
# thuộc vào 1 request cụ thể nào (vd chỉ load bên trong /api/upload-slide), nếu không mọi agent
# gọi LLM (quiz/slide/animation/mindmap) trong 1 process chưa từng nhận request đó sẽ đọc
# os.getenv("API_KEY") ra rỗng và âm thầm rơi vào fallback dù .env đã cấu hình đúng.
load_dotenv()
...
from app.agents.slide_agent import run_slide_agent, stream_slide_narration
from app.agents.animation_agent import run_animation_agent
from app.agents.mindmap_agent import run_mindmap_agent
from app.agents.debate_agent import run_debate_agent
...
app = FastAPI(
    ...
    description="Upload PDF -> sinh outline đa dạng type, mỗi block outline có thể sinh nội dung học tập riêng theo type",
    version="0.1.0",
)
...
# Các type hợp lệ cho từng phần trong outline (phải khớp Literal trong OutlineBlockInput)
...
def generate_slide(block: OutlineBlockInput):
    ...
    return run_slide_agent(block)


# ------------------------------------------------------------------
# Stream lời thuyết trình cho 1 slide (NDJSON, mỗi dòng 1 event):
#   {"type": "meta", ...}   -> gửi 1 lần đầu: ảnh + toạ độ slide gốc để FE dựng khung slide
#   {"type": "focus", ...}  -> đổi vùng đang được khoanh sáng (focus_bbox / null)
#   {"type": "text", ...}   -> chữ để gõ dần (typewriter)
#   {"type": "done"}        -> kết thúc
# ------------------------------------------------------------------
@app.post("/api/generate/slide/stream")
def generate_slide_stream(block: OutlineBlockInput):
    _check_type(block, "slide")
    ...
    def event_gen():
        ...
        for event in stream_slide_narration(block):
...
# ------------------------------------------------------------------
# DEBATE MODE: FE gọi API này 1 lần / 1 lượt phát biểu của 1 agent, kèm theo
# toàn bộ transcript hiện tại (history). Stateless như các endpoint generate/*
# khác — FE giữ transcript và có thể chèn tin nhắn mới của người dùng vào
# giữa các lượt (human-in-the-loop) trước khi gọi lượt kế tiếp.
# ------------------------------------------------------------------
@app.post("/api/debate/turn", response_model=DebateTurnResponse)
def debate_turn(payload: DebateTurnRequest):
    return run_debate_agent(payload)


# ----------------------------------------------------------------------
# UPLOAD SLIDE: nhận file PDF, tách text theo trang, gọi LLM để sinh outline
# đa dạng type (quiz/slide/animation/mindmap). Mỗi phần tử outline trả về
# đúng shape của OutlineBlockInput, để FE có thể POST thẳng vào các API
# /api/generate/{type} phía trên mà không cần biến đổi thêm.
# ----------------------------------------------------------------------
...
@app.post("/api/upload-slide", response_model=UploadSlideResponse)
async def upload_slide(file: UploadFile = File(...)):
    ...
    try:
        ...
        markdown_text = ""
        pages_dict = {}
        # Ảnh render (base64 PNG) của ĐÚNG từng trang gốc -> dùng làm nội dung hiển thị cho
        # slide (thay vì vẽ lại), và làm nền cho outline HTML preview.
        page_bg_images = {}
        ...
        for page_no_0 in range(len(pdf_doc)):
            ...
            try:
                ...
                page_bg_images[page_no] = base64.b64encode(pix.tobytes("png")).decode("utf-8")
            except Exception as e:
                logger.error(f"Lỗi render ảnh nền trang {page_no}: {e}")
                page_bg_images[page_no] = ""
        ...
        # Gom các phần tử text của từng trang gốc, gán id cố định cho mỗi phần tử -> đây là
        # dữ liệu "grounding" để agent slide trỏ narration đúng vào vị trí trên ảnh gốc.
        page_elements = {}
        for item in all_items:
            ...
            if len(bbox) != 4:
                continue
            p = item["page_no"]
            elements_of_page = page_elements.setdefault(p, [])
            elements_of_page.append({
                "id": f"e{p}-{len(elements_of_page) + 1}",
                "text": item.get("text", ""),
        ...
        # ------------------------------------------------------------------
        # Gắn grounding vào slide gốc cho từng block type="slide": nội dung "content" của
        # block do AI viết lại chỉ để tham khảo, còn HIỂN THỊ THỰC TẾ và vị trí khoanh khi
        # thuyết trình phải dựa vào ĐÚNG trang PDF gốc. Vì các block "slide" trong outline
        # được model đọc lần lượt theo đúng thứ tự trang trong markdown gốc, ta gán tuần tự
        # từng block "slide" cho từng trang gốc theo đúng thứ tự xuất hiện.
        # ------------------------------------------------------------------
        page_numbers_sorted = sorted(pages_dict.keys())
        slide_page_cursor = 0
        for item in outline_data:
            if item.get("type") != "slide":
                continue
            if slide_page_cursor < len(page_numbers_sorted):
                p = page_numbers_sorted[slide_page_cursor]
            elif page_numbers_sorted:
                p = page_numbers_sorted[-1]
            else:
                p = None
            slide_page_cursor += 1
            ...
            if p is None:
                continue
            item["page_no"] = p
            item["page_width"] = pages_dict[p]["width"]
            item["page_height"] = pages_dict[p]["height"]
            item["bg_image"] = page_bg_images.get(p, "")
            item["elements"] = page_elements.get(p, [])
        ...
        for page_no, page_info in pages_dict.items():
            ...
            bg_image_b64 = page_bg_images.get(page_no, "")
