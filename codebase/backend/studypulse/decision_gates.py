...
HARD_BOUNDARIES: list[BoundaryRule] = [
    BoundaryRule(
        ...
        user_message_vi="❌ Tôi không có quyền xóa dữ liệu. Vui lòng liên hệ quản trị viên.",
        user_message_en="❌ I cannot delete data. Please contact the administrator.",
    ...
    BoundaryRule(
        ...
        user_message_vi="❌ Không tìm thấy deadline này trong các nguồn chính thức. Tôi không thể tạo deadline không có căn cứ.",
        user_message_en="❌ This deadline was not found in official sources. I cannot create unverified deadlines.",
    ...
    BoundaryRule(
        ...
        user_message_vi="⚠️ Tôi đề xuất thay đổi mục này. Bạn xác nhận? [Có/Không]",
        user_message_en="⚠️ I propose this change. Do you confirm? [Yes/No]",
    ...
    BoundaryRule(
        ...
        user_message_vi="❌ Ngày thi chưa được xác nhận trong thông báo chính thức. Vui lòng kiểm tra với giảng viên.",
        user_message_en="❌ Exam date is not confirmed in official announcements. Please check with your instructor.",
    ...
    BoundaryRule(
        ...
        user_message_vi="❌ Điểm số thuộc hệ thống riêng. Vui lòng kiểm tra trên portal học viên.",
        user_message_en="❌ Grades are managed separately. Please check the student portal.",
    ...
    BoundaryRule(
        ...
        user_message_vi="⚠️ Phản hồi khảo sát phải được lưu nguyên văn. Không thể tóm tắt.",
        user_message_en="⚠️ Survey responses must be saved verbatim. Summarization is not allowed.",
    ...
    BoundaryRule(
        ...
        category="④ Domain-specific",
        boundary_type=BoundaryType.HARD,
        description="Providing study advice or solving homework",
        trigger_condition="User asks AI to solve problems or write essays",
        agent_action="REFUSE. Redirect to learning resources.",
        user_message_vi="❌ Tôi chỉ hỗ trợ quản lý lịch và deadline. Để giải bài tập, hãy sử dụng AI tutor trên VLearn.",
        user_message_en="❌ I only help with schedule and deadline management. For homework help, please use the AI tutor on VLearn.",
        hax_principle="G1 — System capability clarity",
    ),
]
...
# ═══════════════════════════════════════════════════════════════════════════
# ESCALATION TRIGGERS — Require HITL review
# ═══════════════════════════════════════════════════════════════════════════
...
ESCALATION_TRIGGERS: list[BoundaryRule] = [
    BoundaryRule(
        ...
        user_message_vi="⚠️ Mục này có độ tin cậy thấp ({confidence:.0%}). Đã gửi cho TA xác nhận.",
        user_message_en="⚠️ This item has low confidence ({confidence:.0%}). Sent to TA for confirmation.",
    ...
    BoundaryRule(
        ...
        user_message_vi="⚠️ Phát hiện xung đột ngày: {source_a} nói {date_a}, {source_b} nói {date_b}. Vui lòng xác nhận với giảng viên.",
        user_message_en="⚠️ Date conflict detected: {source_a} says {date_a}, {source_b} says {date_b}. Please confirm with instructor.",
    ...
    BoundaryRule(
        ...
        user_message_vi="⚠️ Phát hiện thông báo nhạy cảm. Đã chuyển đến quản trị viên để xem xét.",
        user_message_en="⚠️ Sensitive notification detected. Forwarded to administrator for review.",
    ...
    BoundaryRule(
        ...
        user_message_vi="⚠️ 'Tuần sau' → tôi hiểu là {resolved_date}. Đúng chưa?",
        user_message_en="⚠️ 'Next week' → I interpreted as {resolved_date}. Is this correct?",
    ...
    BoundaryRule(
        ...
        user_message_vi="⚠️ Thông tin này từ kênh chưa xác minh. Cần kiểm tra với nguồn chính thức.",
        user_message_en="⚠️ This information is from an unverified channel. Please verify with official sources.",
    ...
    BoundaryRule(
        ...
        user_message_vi="⚠️ Không thể trích xuất sau 3 lần thử. Đã chuyển cho TA xử lý thủ công.",
        user_message_en="⚠️ Extraction failed after 3 attempts. Forwarded to TA for manual processing.",
...
FALLBACK_STRATEGIES: list[FallbackStrategy] = [
    FallbackStrategy(
        ...
        message_vi="⏳ Hệ thống đang tải. Vui lòng thử lại sau 30 giây hoặc liên hệ TA.",
        message_en="⏳ System is loading. Please retry in 30 seconds or contact TA.",
    ...
    FallbackStrategy(
        ...
        message_vi="📭 Không tìm thấy mục mới từ nguồn này. Kiểm tra lại hộp thư hoặc Discord trực tiếp.",
    ...
    FallbackStrategy(
        ...
        message_vi="🔍 Không tìm thấy tài liệu phù hợp. Thử tìm kiếm với từ khóa khác hoặc hỏi TA.",
    ...
    FallbackStrategy(
        ...
        message_vi="🔒 Lỗi xử lý bảo mật. Không thể hiển thị kết quả. Đã thông báo quản trị viên.",
    ...
    FallbackStrategy(
        ...
        message_vi="🌐 Hiện tại tôi hỗ trợ Tiếng Việt và Tiếng Anh. Vui lòng nhập lại bằng một trong hai ngôn ngữ.",
