...
type GenerateQuizResponse = { outline: OutlineSection[]; questions: RawBackendQuestion[] };

// ---- QUYẾT ĐỊNH AI TRUNG TÂM: gọi backend thật (OpenAI), không hardcode ----
export async function generateQuiz(): Promise<{ outline: OutlineSection[]; questions: McqQuestion[] }> {
  const res = await fetch(`${BACKEND_URL}/api/quiz/generate?n_questions=20`, { method: "POST" });
  if (!res.ok) {
...
export async function gradeQuiz(questions: McqQuestion[], answers: string[]): Promise<GradeResult> {
  ...
  questions.forEach((q, i) => {
    const stat = bySectionMap.get(q.section) ?? { section: q.section, correct: 0, total: 0 };
    stat.total += 1;
...
export async function getStudyContent(
  ...
  return sections.map((sectionId) => {
    ...
    return {
      ...
      example: "(Mock — bước sinh ví dụ thực tế bằng AI chưa build, xem docs/product-overview.md)",
      practice: poolQuestion
        ? { question: poolQuestion.question, options: poolQuestion.options, answer: poolQuestion.answer }
        : null,
...
export async function generateRetest(
  ...
  return sections.flatMap((s) =>
    questionPool
      ...
      .slice(0, perSection)
      .map((q) => ({ ...q, id: `${q.id}-retest`, question: `Ôn lại: ${q.question}` })),
  );
}
