---
course: packs
generated: '2026-07-30T10:32:43+00:00'
lang: vi
...
source_hash: sha256:60dbb102c73fb70dadef120eb6a798cee314866e2ce597e04197afb96d4cd826
type: lesson-note
---
...
| Flexibility Memory | Thấp                   | Gần như không có              | Cao                        |
| Tool use          | Hard-coded             | Có thể gọi tool theo chỉ định | Chủ động chọn tool         |
| Cost              | Thấp nhất              | Cao hơn                       | Cao hơn                    |
...
- [[Rule-based-bot]]: Trả menu lựa chọn cố định, không search được dữ liệu mới.  
...
State và memory giúp agent không "mất mạch". 4 khối kiến trúc thường kéo theo 4 nhóm cost chính: token, storage, API, và latency.
...
## Slide 21 — Định Nghĩa
ReAct = Reasoning + Acting: pattern kết hợp suy luận theo từng bước với gọi công cụ và quan sát kết quả.  
Thay vì trả lời ngay, agent sẽ lặp qua các bước:  
...
def recommend_outfit(temp_high: int, rain_probability: float) -> str:
    if rain_probability > 0.5:
        return "Áo mỏng, giày dễ khô, mang theo ô gập."
    if temp_high > 30:
...
|------------------|----------------------------------------|--------------------------------------|
| Tác vụ           | FAQ, support đơn giản, nội dung 1 lượt | Booking, research, coding           |
| Tốc độ           | Nhanh, ít round-trip                   | Chậm hơn do loop và tool calls      |
| Cost             | Thấp hơn, predictable hơn              | Cao hơn nhưng xử lý được bài toán khó hơn |
...
Thiết kế tốt thường là: triage nhanh, câu đơn giản đi chatbot path, câu phức tạp mới mở agent loop.
...
## Khái niệm chính
- [[rule-based-bot]]: Hệ thống chatbot thực hiện theo quy tắc cứng nhắc, không thích ứng với ngữ cảnh.
- [[LLM-chatbot]]: Hệ thống chatbot sử dụng mô hình ngôn ngữ lớn để tạo phản hồi sáng tạo hơn.
- [[agent]]: Hệ thống tự động hiểu và hành động dựa trên quan sát, có khả năng quyết định.
- [[ReAct]]: Kiểu mẫu kết hợp suy luận và hành động tạo nên sự tương tác hiệu quả giữa agent và môi trường.
- [[Agentic-Fit]]: Khung phân tích để xác định xem một bài toán có cần được giải quyết bởi agent hay không.
- [[multi-step-reasoning]]: Quá trình suy luận yêu cầu nhiều bước và phụ thuộc lẫn nhau.
- [[tool-use]]: Sự khai thác các công cụ bên ngoài để hoàn thành nhiệm vụ.
- [[dynamic-decisions]]: Quyết định cần được điều chỉnh liên tục dựa trên thông tin mới.
- [[long-horizon]]: Mục tiêu dài hạn đòi hỏi sự duy trì sự chú ý và điều chỉnh qua nhiều bước.
```
