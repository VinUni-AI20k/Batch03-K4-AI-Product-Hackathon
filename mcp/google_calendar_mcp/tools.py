...
@mcp.tool(
    ...
    "'timezone' (IANA name, e.g. 'Asia/Ho_Chi_Minh') is used instead.",
)
async def create_event(
    ...
    service = calendar_client.get_service()
    event = await calendar_client.execute(service.events().insert(calendarId=cal_id, body=body))
...
@mcp.tool(
    ...
    "start/end follow the same format as create_event.",
)
async def update_event(
    ...
    if not event_id:
        raise ValueError("eventId cannot be empty")
    if not any([summary, start, end, description, location]):
    ...
    event = await calendar_client.execute(
        service.events().patch(calendarId=cal_id, eventId=event_id, body=body)
    )
